...
#     x1, y1 = point1
#     x2, y2 = point2
#     s = np.array(image).shape[0]
#     print('before_coordinate: ', x1, y1, x2, y2)
#     x1 = s * (x1 - x2 / 2)
#     y1 = s * (y1 - y2 / 2)
#     x2 = s * (x1 + x2 / 2)
#     y2 = s * (y1 + y2 / 2)
#     print('after_coordinate: ', x1, y1, x2, y2)
#     point1 = (x1, y1)
#     point2 = (x2, y2)
...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        # print(img, targets, paths, shapes)
        # rec_img = Image.fromarray(np.array(torch.permute(img[0, :, :, :].squeeze(), (1, 2, 0))))
        # print('targets_shape', targets.shape)
        # for label in targets:
        #     x1, y1 = label[2:4]
        #     x2, y2 = label[4:6]
        #     rec_img = pil_draw_rect(rec_img, (x1, y1), (x2, y2))
        # print(torch.permute(img[0, :, :, :].squeeze(), (1, 2, 0)))
        # cv2.imwrite('./val_img.jpg', np.array(rec_img))
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/yolo.yaml', help='dataset.yaml path')
