...
from utils.general import check_dataset, check_requirements, check_yaml, clean_str, segments2boxes, \
    xywh2xyxy, xywhn2xyxy, xyxy2xywhn, xyn2xy, save_one_box
...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # print('mosaic_img_size: ', img.shape)
            shapes = None
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            labels[:, 1:] = xywhn2xyxy(labels[:, 1:], w, h, padw, padh, s)  # normalized xywh to pixel xyxy format
            segments = [xyn2xy(x, w, h, padw, padh) for x in segments]

        # rec_img = Image.fromarray(np.array(img))
        # for label in labels:
        #     x1, y1 = label[1:3]
        #     x2, y2 = label[3:5]
        #     rec_img = pil_draw_rect(rec_img, (x1, y1), (x2, y2))
        # cv2.imwrite('./mosaic.jpg', np.array(rec_img))
